    ...
    <section class="content">
      <div class="box box-danger">
      	<div class="box-header with-border">
            ...
            <label>用户：</label>
            <input type="text" name="username" placeholder="请输入用户名..." value="${param.username }" style="padding: 4px 12px;margin-right: 10px;"/>
            <label>真实姓名：</label>
            <input type="text" name="realname" placeholder="请输入真实姓名..." value="${param.realname }" style="padding: 4px 12px;margin-right: 10px;"/>
            <label>邮箱：</label>
            <input type="text" name="email" placeholder="请输入邮箱..." value="${param.email }" style="padding: 4px 12px;margin-right: 10px;"/>
            <label>手机：</label>
            <input type="text" name="phoneNum" placeholder="请输入手机号..." value="${param.phoneNum }" style="padding: 4px 12px;margin-right: 10px;"/>
            
            <button type="submit" class="btn btn-default btn-flat" style="margin-right: 10px;"><i class="fa fa-search"></i> 查寻</button>
         	<a href="${ctx }/admin/edit" class="btn btn-info btn-flat">
         ...
         <div class="box-body">
           ...
           <c:forEach var="u" items="${pageModel.list }">
             <tr>
               ...
               <td>
               	 <a href="${ctx }/admin/edit?id=${u.id}" class="btn btn-xs btn-info btn-flat">修改</a>
               	 <c:if test="${u.status == 1 }">
               	  <a href="${ctx }/admin/changeStatus?id=${u.id}&status=0" class="btn btn-xs btn-danger btn-flat">禁用</a>
               	 </c:if>
               	 <c:if test="${u.status == 0 }">
               	  <a href="${ctx }/admin/changeStatus?id=${u.id}&status=1" class="btn btn-xs btn-success btn-flat">启用</a>
               	 </c:if>
               </td>
             </tr>
           </c:forEach>
